...
const TASK_PROFILES = Object.freeze({
  task1: {
    ...
    title: "药盒装箱 · 单盒 7 步流程",
    summary:
      "按固定7步流程循环18盒；第5步识别竖置药盒的带条码顶部窄面并由左吸盘吸取。",
    ...
    steps: [
      ...
      [
        ...
        "已接入 · 并行识别并缓存",
        true,
        "药盒识别与纸箱 RGB-D 20槽识别并行启动；两项都完成后同时缓存抓取点、下一空槽及右臂放置中心，不移动机械臂。",
      ],
      [
        "药盒吸取",
        "已接入",
      ...
      [
        ...
        true,
        "左右臂先并行到各自 zhuangxiang 保存位姿并校验双回执；删除原始前54个静止帧后，从原始第55帧开始重放奇数帧轨迹，原始第251帧仍在同一动作节点自动关闭吸盘。右夹爪反馈继续记录，但不再因误差中止此专用轨迹。",
      ],
      [
        "识别并吸取药盒顶部",
        "已接入 · 固定吸取位 + 右臂清障",
        true,
        "左臂使用操作员示教的固定接触位吸附；等待1秒后完全张开右夹爪，等待0.5秒，再让右臂沿右基座Y负方向移动150毫米，最后左臂竖直抬升120毫米。",
      ],
      [
        "放入箱子",
        "已接入 · 双臂并行",
        true,
        "使用第2步缓存的下一空槽让左臂放入药盒；同时右臂沿Z正方向抬升150毫米，随后回到工程页面中的右臂系统初始位姿。",
      ],
      [
        "前推、松开吸盘、上抬",
        "待实现",
...
const WORKFLOW_STEP_DESCRIPTIONS = Object.freeze({
  ...
  detectTask1Carton: "并行运行药盒主面识别与纸箱四内边线 RGB-D 20槽识别；等待两项完成后同时缓存抓取点、下一空槽和右臂放置中心，不移动机械臂。",
  ...
  pickTask1StagedTop: "左臂使用操作员示教的固定吸取位，接近距离80毫米；吸附后等待1秒，右夹爪完全张开，等待0.5秒并复核稳定反馈，右臂沿右基座Y负方向移动150毫米，随后左臂以DEFAULT速度单次竖直抬升120毫米，并只在最终位置确认到位。",
  placeTask1InBox: "左臂使用第2步缓存的下一空槽执行放箱并保持吸盘开启；右臂同时沿Z正方向移动150毫米，随后回到工程页面中的右臂系统初始位姿。",
  placeTask1FixedTrajectory: "保持左吸盘吸合，左右臂并行到各自 zhuangxiang 保存位姿并校验双回执；删除原始前54个静止帧，从原始第55帧开始以2×速度重放268个奇数帧。吸盘仍在原始第251帧的同一双臂动作节点自动关闭。此专用轨迹不使用关节或右夹爪跟随误差中止；右夹爪命令与反馈仍逐帧执行和记录，速度及步长保护保留。",
...
async function executeWorkflowStep() {
  ...
  try {
    ...
    state.message = `第 ${index + 1} 步已完成，机械臂已停住。`;
...
async function startRecording() {
  if (!appState.serviceOnline || appState.recordingBusy || recordingIsActive()) {
    return;
  }
...
function updateCartesianJogControls() {
  ...
  el["left-arm-reset-home"].disabled =
    generallyBlocked || !homeAvailable || recordingIsActive();
  el["right-arm-reset-home"].disabled =
    generallyBlocked || !rightHomeReady || recordingIsActive();
...
function updateRecordingButtons() {
  ...
  el["recording-label"].disabled = active || appState.recordingBusy;
  el["recording-purpose"].disabled = active || appState.recordingBusy;
