            ...
            with self._arm_session() as arm:
                ...
                if not self._dry_run:
                    ...
                    for target_index, target in enumerate(targets):
                        ...
                        elif moved is not True:
                            # AIRBOT's blocking Cartesian call can return None
                            # even after the arm reaches the target.  For an
                            # intentionally unverified intermediate waypoint,
                            # use one feedback sample to distinguish that SDK
                            # ambiguity from a real rejection without adding a
                            # full stable-sample wait between lift segments.
                            actual_position, actual_quaternion = self._read_pose(arm)
                            final_position_error = _position_error_m(
                                actual_position,
                                target,
                            )
                            final_orientation_error = _quaternion_error_deg(
                                actual_quaternion,
                                locked_quaternion,
                            )
                            if (
                                final_position_error > self._max_position_error_m
                                or final_orientation_error
                                > self._max_orientation_error_deg
                            ):
                                raise CartesianJogUnavailable(
                                    f"{operation} target was not reached after an "
                                    "indeterminate SDK result "
                                    f"(position_error={final_position_error * 1000.0:.3f} "
                                    "mm, orientation_error="
                                    f"{final_orientation_error:.3f} deg)"
                                )
