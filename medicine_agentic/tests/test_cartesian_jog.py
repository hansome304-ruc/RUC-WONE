...
class FakeArm:
    def __init__(self) -> None:
        ...
        self.gripper_moves: list[tuple[float, bool]] = []
    ...
    def move_to_cart_pose(self, target, *, blocking):
        ...
        if self.move_result is not False and self.follow_target:
    ...
    def move_eef_pos(self, target, *, blocking):
        self.gripper_moves.append((float(target), blocking))
        if self.move_result and self.follow_target:
...
def controller(
    ...
    *,
    ...
    endpoint=None,
    manual_jog_workspace_enforced=True,
    calibrated_workspace_floor_z_m_by_profile=None,
    ...
    config = {
        ...
        "manual_jog_workspace_enforced": manual_jog_workspace_enforced,
    ...
    if calibrated_workspace_floor_z_m_by_profile is not None:
        config["calibrated_workspace_floor_z_m_by_profile"] = (
    ...
    instance = CartesianJogController(
        ...
        slow_speed="SLOW",
        speed_profiles={"DEFAULT": "DEFAULT", "FAST": "FAST"},
    )
...
class CartesianJogTests(unittest.TestCase):
    def test_read_current_pose_only_reads_feedback(self) -> None:
        ...
        self.assertEqual(pose["gripper_position_m"], arm.gripper_position)
        self.assertTrue(pose["read_only"])
        self.assertEqual(arm.moves, [])
        self.assertEqual(arm.joint_moves, [])

    def test_read_current_pose_does_not_require_motion_workspace(self) -> None:
        arm = FakeArm()
        factory = FakeFactory(arm)
        jog = CartesianJogController(
            {
                "enabled": True,
                "dry_run": False,
        ...
        self.assertEqual(factory.calls, [("localhost", 50053)])
        self.assertEqual(pose["arm"], "right")
        self.assertEqual(pose["joint_positions_rad"], arm.joint_positions)
    ...
    def test_manual_jog_can_disable_all_absolute_position_limits(self) -> None:
        arm = FakeArm()
        jog, _factory = controller(
            arm,
            manual_jog_workspace_enforced=False,
        )
        jog.capture_orientation()
        jog.enable("dry", area_clear=True, estop_ready=True)
        arm.position = [2.0, -2.0, 0.004]

        result = jog.jog("z", -5)

        self.assertEqual(result["target_position_m"], [2.0, -2.0, -0.001])
        self.assertFalse(jog.status()["manual_jog_workspace_enforced"])

    def test_bounded_skill_can_use_sampled_z_slightly_below_jog_floor(self) -> None:
        arm = FakeArm()
        capture_workspace = {**WORKSPACE, "z_min": 0.0}
        jog, _factory = controller(
            arm,
        ...
        with self.assertRaises(CartesianJogSafetyViolation):
            jog.move_fixed_orientation_path([[0.30, 0.0, 0.078]])

        jog.capture_orientation()
        jog.enable("dry", area_clear=True, estop_ready=True)
        result = jog.move_fixed_orientation_path(
            [[0.30, 0.0, 0.078]],
            operation="task3_pick_approach",
            calibrated_minimum_z_m=0.078,
        )
        self.assertTrue(result["dry_run"])
        self.assertEqual(result["effective_minimum_z_m"], 0.078)

        jog.capture_orientation()
        jog.enable("dry", area_clear=True, estop_ready=True)
        exact_boundary = jog.move_fixed_orientation_path(
            [[0.30, 0.0, 0.075]],
            operation="task3_pick_exact_5mm_relaxation",
            calibrated_minimum_z_m=0.075,
        )
        self.assertTrue(exact_boundary["dry_run"])
        self.assertEqual(exact_boundary["effective_minimum_z_m"], 0.075)

        with self.assertRaises(CartesianJogSafetyViolation):
            jog.move_fixed_orientation_path(
                [[0.30, 0.0, 0.070]],
                operation="task3_pick_approach",
                calibrated_minimum_z_m=0.070,
            )

    def test_named_task1_profile_allows_only_configured_negative_floor(self) -> None:
        arm = FakeArm()
        jog, _factory = controller(
            arm,
            calibrated_workspace_floor_z_m_by_profile={
        ...
        result = jog.move_fixed_orientation_path(
            [[0.30, 0.0, -0.085]],
            operation="task1_pick_approach",
            calibrated_workspace_profile="task1_pick",
        )

        self.assertTrue(result["dry_run"])
        self.assertEqual(result["effective_minimum_z_m"], -0.10)
        with self.assertRaises(CartesianJogSafetyViolation):
            jog.move_fixed_orientation_path(
                [[0.30, 0.0, -0.101]],
                operation="task1_pick_below_calibrated_floor",
                calibrated_workspace_profile="task1_pick",
            )
        with self.assertRaises(CartesianJogSafetyViolation):
            jog.move_fixed_orientation_path(
                [[0.30, 0.0, -0.085]],
                operation="task1_pick_without_profile",
            )
    ...
    def test_real_jog_locks_orientation_mode_speed_and_blocking(self) -> None:
        ...
        self.assertEqual(arm.speed, "DEFAULT")
    ...
    def test_fixed_orientation_path_fails_when_rejection_is_confirmed_by_feedback(
        self,
    ) -> None:
        arm = FakeArm()
        arm.move_result = False
        jog, _factory = controller(arm, dry_run=False)
        jog.capture_orientation()
        jog.enable("operator-secret", area_clear=True, estop_ready=True)
        ...
        with self.assertRaisesRegex(
            CartesianJogSafetyViolation,
            "feedback did not settle",
        ):
            jog.move_fixed_orientation_path(
                [[0.30, 0.0, 0.325], [0.30, 0.0, 0.40]],
                operation="fast_lift",
                speed_profile="FAST",
            )

        self.assertEqual(arm.speed, "FAST")
        self.assertEqual(len(arm.moves), 1)

    def test_fixed_orientation_path_accepts_none_when_feedback_reaches_targets(
        self,
    ) -> None:
        arm = FakeArm()
        arm.move_result = None
        jog, _factory = controller(arm, dry_run=False)
        jog.capture_orientation()
        jog.enable("operator-secret", area_clear=True, estop_ready=True)
        ...
        result = jog.move_fixed_orientation_path(
            [[0.30, 0.0, 0.325], [0.30, 0.0, 0.40]],
            operation="default_lift",
            speed_profile="DEFAULT",
            verify_each_target=False,
        )

        self.assertTrue(result["executed"])
        self.assertEqual(result["sdk_reported_success_by_target"], [False, False])
        self.assertEqual(len(arm.moves), 2)
        self.assertEqual(arm.position, [0.30, 0.0, 0.40])

    def test_fast_path_can_verify_only_the_final_target(self) -> None:
        arm = FakeArm()
        jog, _factory = controller(arm, dry_run=False)
        jog.capture_orientation()
        jog.enable("operator-secret", area_clear=True, estop_ready=True)
        ...
        self.assertTrue(result["executed"])
        self.assertFalse(result["verify_each_target"])
        self.assertEqual(result["speed_profile"], "FAST")
        self.assertEqual(len(arm.moves), 2)
        self.assertEqual(arm.position, [0.30, 0.0, 0.40])

    def test_fixed_orientation_entry_reaches_approach_and_arms_jog(self) -> None:
        arm = FakeArm()
        arm.position = [0.22, -0.12, 0.24]
        arm.quaternion = [0.0, 0.0, 0.0, 1.0]
        jog, _factory = controller(arm, dry_run=False)
        target_position = [0.34, -0.18, 0.12]
        target_quaternion = [0.0, 0.70710678, 0.0, 0.70710678]

        result = jog.move_to_fixed_orientation_entry(
            target_position,
            target_quaternion,
            transit_z_m=0.30,
            enable_token="operator-secret",
        ...
        self.assertTrue(result["executed"])
        self.assertEqual(result["operation"], "task2_direct_pick_entry")
        self.assertEqual(arm.waypoint_moves, [])
        self.assertEqual(len(arm.moves), 3)
        self.assertTrue(all(blocking for _target, blocking in arm.moves))
        self.assertEqual(arm.moves[0][0][0], [0.22, -0.12, 0.30])
        self.assertEqual(arm.moves[1][0][0], [0.34, -0.18, 0.30])
        self.assertEqual(arm.moves[-1][0][0], target_position)
        self.assertEqual(result["planner"], "verified_planning_pos_segments")
        self.assertEqual(result["planned_target_count"], 3)
        self.assertEqual(result["completed_targets_m"][-1], target_position)
        self.assertEqual(arm.mode, "PLANNING_POS")
        status = jog.status()
        self.assertTrue(status["enabled"])
        self.assertEqual(status["current_position_m"], target_position)
        self.assertLess(
            abs(sum(a * b for a, b in zip(
                status["locked_quaternion_xyzw"],
                target_quaternion,
            ))),
            1.000001,
        )

    def test_fixed_orientation_entry_uses_named_task1_negative_floor(self) -> None:
        arm = FakeArm()
        arm.position = [0.22, -0.12, 0.24]
        arm.quaternion = [0.0, 0.0, 0.0, 1.0]
        jog, _factory = controller(
        ...
        target_position = [0.34, -0.18, -0.060]
        ...
        self.assertTrue(result["executed"])
        self.assertEqual(result["effective_minimum_z_m"], -0.10)
        self.assertEqual(result["calibrated_workspace_profile"], "task1_pick")
        self.assertEqual(result["completed_targets_m"][-1], target_position)

    def test_fixed_orientation_entry_rejects_bad_token_without_motion(self) -> None:
        arm = FakeArm()
        jog, _factory = controller(arm, dry_run=False)

        with self.assertRaises(CartesianJogSafetyViolation):
            jog.move_to_fixed_orientation_entry(
                [0.34, -0.18, 0.12],
                [0.0, 0.70710678, 0.0, 0.70710678],
                transit_z_m=0.30,
                enable_token="wrong",
    ...
    def test_fixed_orientation_entry_does_not_force_downward_pose_at_watcher_height(self) -> None:
        arm = FakeArm()
        arm.position = [-0.02, -0.02, 0.43]
        arm.quaternion = [-0.03, -0.77, -0.10, 0.63]
        jog, _factory = controller(
            arm,
            dry_run=False,
            workspace={
        ...
        target_position = [0.184, -0.240, 0.05]
        target_quaternion = [0.0, 0.70710678, 0.0, 0.70710678]
        ...
        self.assertTrue(result["executed"])
        self.assertEqual(result["transit_z_m"], 0.30)
        self.assertEqual(result["planned_target_count"], 2)
        self.assertEqual(len(arm.moves), 2)
        self.assertEqual(arm.moves[0][0][0], [0.184, -0.240, 0.30])
        for actual, expected in zip(arm.moves[0][0][1], target_quaternion):
            self.assertAlmostEqual(actual, expected)
        self.assertEqual(arm.moves[1][0][0], target_position)
    ...
    def test_reset_home_accepts_verified_fast_profile(self) -> None:
        arm = FakeArm()
        jog, _factory = controller(arm, dry_run=False)

        result = jog.reset_home(speed_profile="FAST")
    ...
    def test_saved_pose_accepts_verified_fast_profile(self) -> None:
        arm = FakeArm()
        jog, _factory = controller(arm, dry_run=False)
        target = [0.1, -0.2, 0.3, 1.4, -0.1, -1.3]

        result = jog.move_to_saved_joint_pose(
            target,
    ...
    def test_saved_pose_restores_recorded_gripper_target(self) -> None:
        arm = FakeArm()
        jog, _factory = controller(arm, dry_run=False)
        target = [0.1, -0.2, 0.3, 1.4, -0.1, -1.3]
        ...
        result = jog.move_to_saved_joint_pose(
            target,
            pose_name="right_init",
            gripper_position_m=0.0,
        )

        self.assertEqual(arm.gripper_moves, [(0.0, True)])
        self.assertEqual(result["target_gripper_position_m"], 0.0)
        self.assertEqual(result["actual_gripper_position_m"], 0.0)

    def test_gripper_only_move_does_not_command_arm_joints_or_cartesian_pose(
        self,
    ) -> None:
        arm = FakeArm()
        jog, _factory = controller(arm, dry_run=False)

        result = jog.move_gripper_to_position(
            0.06,
        ...
        self.assertTrue(result["executed"])
        self.assertEqual(result["arm"], "left")
        self.assertEqual(result["target_gripper_position_m"], 0.06)
        self.assertEqual(result["actual_gripper_position_m"], 0.06)
        self.assertEqual(arm.gripper_moves, [(0.06, True)])
        self.assertEqual(arm.joint_moves, [])
        self.assertEqual(arm.moves, [])
        self.assertEqual(arm.waypoint_moves, [])
