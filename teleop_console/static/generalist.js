(() => {
  ...
  const basePresets = {
    // The former "fast" setting is now the baseline slow gear requested for
    // this platform. Medium/fast remain below the chassis hard limits
    // (0.20 m/s and 0.50 rad/s) and use the same bounded deadman pulse.
    slow: { speed: 0.08, yaw_speed: 0.25, duration: 0.40 },
    medium: { speed: 0.11, yaw_speed: 0.32, duration: 0.40 },
    fast: { speed: 0.14, yaw_speed: 0.38, duration: 0.40 },
  };
  ...
  const ARM_SERVICE_JOB_TIMEOUT_MS = 180000;
  ...
  const followerSideState = (side, state, text) => {
    const label = side === "left" ? "L" : "R";
    serviceState(`gen-follower-${side}-inline-status`, state, `${label} ${text}`);
  };

  const followActionLabels = {
    start: "启动中",
    stop: "停止中",
    check: "检查中",
  };

  const renderFollowOperationState = (action) => {
    const label = followActionLabels[action] || "执行中";
    dot("gen-follow-dot", null);
    byId("gen-follow-summary").textContent = `Master Follow ${label}`;
    serviceState("gen-follow-inline-status", "pending", label);
  };
  ...
  function renderReadiness() {
    document.querySelectorAll(
      ...
      const armService = button.dataset.genArmService?.split("-")[0] || "";
      if (armService && generalist.armServiceJobs.has(armService)) {
        button.disabled = true;
        return;
      }
      button.disabled = safeWhileBlocked
        ? false
        : generalist.halted || chargingBlocksBase;
    });
  ...
  function renderSystemStatus(status) {
    ...
    generalist.health.follow = Boolean(status.teleop?.running);
    ...
    dot("gen-follow-dot", generalist.health.follow);
    const followRecovering = Boolean(
      status.teleop?.desired && !status.teleop?.running,
    );
    const followPairs = status.teleop?.pairs || {};
    const pairState = (side, label) => {
      const pair = followPairs[side] || {};
      if (pair.healthy) return `${label} 已同步`;
      if (pair.alive || status.teleop?.desired) return `${label} 恢复中`;
      return `${label} 未运行`;
    };
    const pairEefState = (side) => {
      const pair = followPairs[side] || {};
      const eef = pair.eef || {};
      if (pair.endpoint_recovery_pending) return "驱动恢复中";
      if (eef.state === "opening_stalled") return "开合无响应";
      if (eef.state === "feedback_wait") return "反馈等待";
      if (eef.state === "closing_contact_or_stall") return "夹持/受阻";
      if (pair.healthy && eef.feedback_m != null) return "正常";
      return "-";
    };
    byId("gen-follow-summary").textContent = generalist.health.follow
      ? "Master Follow 双臂同步运行"
      : followRecovering
        ? "Master Follow 自动恢复中"
        : "Master Follow 未运行";
    byId("gen-follow-detail").innerHTML = miniRows([
      ["架构", status.teleop?.architecture === "independent-factory-pairs" ? "厂家双独立进程" : "兼容模式"],
      ["Follow 进程", generalist.health.follow ? "健康" : followRecovering ? "等待/重连" : "未运行"],
      ["左侧", pairState("left", "L")],
      ["右侧", pairState("right", "R")],
      ["心跳", status.teleop?.heartbeat_age_s == null ? "-" : `${status.teleop.heartbeat_age_s}s`],
      [
        "单侧重启",
        `L ${status.teleop?.pair_restarts?.left || 0} / R ${status.teleop?.pair_restarts?.right || 0}`,
      ],
      [
        "夹爪闭环",
        `L ${pairEefState("left")} / R ${pairEefState("right")}`,
      ],
      [
        "夹爪驱动恢复",
        `L ${status.teleop?.endpoint_recoveries?.left || 0} / R ${status.teleop?.endpoint_recoveries?.right || 0}`,
      ],
    ]);
    serviceState(
      "gen-follow-inline-status",
      generalist.health.follow ? "ready" : followRecovering ? "pending" : "idle",
      generalist.health.follow ? "双臂同步" : followRecovering ? "恢复中" : "未运行",
    );
    if (generalist.followServiceJob) {
      renderFollowOperationState(generalist.followServiceJob.action);
    }
  ...
  function renderArmServices(payload) {
    ...
    const followerBySide = new Map(
      followerArms.map((arm, index) => [
        arm.side || (index === 0 ? "left" : "right"),
        arm,
      ]),
    );
    for (const side of ["left", "right"]) {
      const arm = followerBySide.get(side);
      const connected = Boolean(arm?.port_open);
      const canReady = Boolean(arm?.can_up);
      followerSideState(
        side,
        connected ? "ready" : canReady && follower.session_running ? "pending" : "idle",
        connected ? "已连接" : canReady && follower.session_running ? "连接中" : "未连接",
      );
    }
    ...
    generalist.armServiceJobs.forEach((job, service) => {
      const label = armServiceActionLabels[job.action] || "执行中";
      if (service === "follower") {
        followerSideState("left", "pending", label);
        followerSideState("right", "pending", label);
      } else {
        serviceState(`gen-${service}-inline-status`, "pending", label);
      }
    });
  ...
  async function refreshSystem() {
    ...
    if (armServicesResult.status === "fulfilled") {
      ...
      followerSideState("left", "idle", "读取失败");
      followerSideState("right", "idle", "读取失败");
    }
    renderReadiness();
  }
  ...
  async function runArmService(command) {
    ...
    renderArmServices(generalist.armServices || {});
    renderReadiness();
    try {
  ...
  async function runService(action) {
    ...
    if (generalist.followServiceJob) {
      throw new Error("主从跟随服务操作正在执行");
    }
    if (action.endsWith("-start") && generalist.halted) {
      throw new Error("安全状态未明确复位，主从遥操作启动已阻止");
    }
    generalist.followServiceJob = {
      id: "",
      action: serviceAction,
    };
    renderFollowOperationState(serviceAction);
    try {
      ...
      if (!payload.job) {
        generalist.followServiceJob = null;
        await refreshSystem();
        refreshJobs().catch(() => {});
        return;
      }

      generalist.followServiceJob.id = payload.job.id;
      state.selectedJobId = payload.job.id;
      window.dispatchEvent(new CustomEvent("ruc:job-started", { detail: payload.job }));
      refreshJobs().catch(() => {});
      ...
      while (
        ...
        const jobPayload = await api(`/api/jobs/${encodeURIComponent(payload.job.id)}`);
        currentJob = jobPayload.job || currentJob;
      }

      if (generalist.followServiceJob?.id === payload.job.id) {
        generalist.followServiceJob = null;
      }
      await Promise.allSettled([refreshSystem(), refreshJobs()]);
      window.dispatchEvent(new CustomEvent("ruc:terminal-refresh"));
      ...
      if (currentJob.status === "success") {
        toast(`主从跟随${actionLabel}完成`);
      } else if (ARM_SERVICE_TERMINAL_STATES.has(currentJob.status)) {
        toast(`主从跟随${actionLabel}失败，请查看运行日志`);
      } else {
        toast(`主从跟随${actionLabel}仍在执行，状态将继续自动刷新`);
      }
    } catch (error) {
      generalist.followServiceJob = null;
  ...
  window.addEventListener("ruc:teleop-safety-state", (event) => {
    const detail = event.detail || {};
    generalist.halted = detail.latched !== false;
    generalist.estopState = generalist.halted ? "teleop-latched" : "clear";
    generalist.estopConfirmed = generalist.halted;
    if (generalist.halted) {
      stopDrive({ sendStop: false });
      stopLiftHold({ sendStop: false });
    }
  ...
  if (state.teleopSafety) {
    generalist.halted = state.teleopSafety.latched !== false;
    generalist.estopState = generalist.halted ? "teleop-latched" : "clear";
    generalist.estopConfirmed = generalist.halted;
  }
